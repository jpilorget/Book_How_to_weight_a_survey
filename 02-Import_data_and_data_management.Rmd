...
paquetes <- c("RCurl", "MASS", "glmnet", "caret", "survey", "readxl", "tidyverse", "foreign", "magrittr", "essurvey")
...
options(scipen = 9999, dplyr.width = Inf)
...
The following chunk of code loads the data sets from a *data* folder in the working directory. The sample data file is stored into the *sample_data* 'data_frame' object. The contact forms information is stored into the *paradata* folder. Survey responses from the integrated interviewer file are saved in the *responses* object. We also store the weight variables included in the integrated interviewer file in the *original_weights* data_frame.

```{r data_files, cache=TRUE}

set_email("user@email.com")
download_country("United Kingdom", 7, format = "spss")

sample_data <- read.spss("data/ESS7SDDFe1_1.sav", to.data.frame = T)  %>%
  filter(cntry == "United Kingdom")

paradata <- read.spss("data/ess7CFe02_1.sav", to.data.frame = T) %>%
  filter(cntry == "United Kingdom") 

responses <- read.spss("data/ESS7INTe02_1.sav", to.data.frame = T) %>%
  filter(cntry == "United Kingdom") 

original_weights <- responses %>% 
  dplyr::select(idno, dweight, pspwght, pweight)
...
vars_sample_data <- c("idno", "psu", "prob")

vars_paradata <- c("idno", "typesamp", "interva", "telnum", "agea_1", "gendera1", "type", "access",
                   "physa", "littera", "vandaa")

resp_id <- c("idno")

resp_x <- c("vote", "prtvtbgb", "prtclbgb", "prtdgcl", "ctzcntr", "ctzshipc", "brncntr","cntbrthc", "gndr", "agea", "hhmmb","eisced", "region", "pdwrk", "edctn", "uempla", "uempli", "rtrd", "wrkctra", "hinctnta")

resp_y <- c("cgtsmke", "cgtsday", "alcfreq", "alcwkdy", "alcwknd")
```
...
sample_data <- 
  sample_data  %>%
  select(vars_sample_data)

paradata <- 
  paradata %>% 
    select(vars_paradata) 

responses <- 
  responses %>% 
  select(resp_id, resp_x, resp_y)
```
